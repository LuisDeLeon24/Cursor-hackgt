...
function initDragFallback() {
  ...
  const move = (x, y) => {
    ...
    App.state.targetRotacion = ((App.state.targetRotacion - (x - lastX) * 0.35) % 360 + 360) % 360;
    App.state.targetElevacion = Math.max(-80, Math.min(80, App.state.targetElevacion + (y - lastY) * 0.22));
    lastX = x; lastY = y;
  };
